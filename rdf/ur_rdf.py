...
class BPSDF():
    ...
    def train_bf_sdf(self, epoches=200):
        ...
        for i, link in enumerate(link_order):
            mesh_name = link
            mesh_file = os.path.join(CUR_DIR, "../ur3e/model", f"{mesh_name}.stl")
            if not os.path.exists(mesh_file):
                print(f"[Warning] mesh file {mesh_file} not found, skipping {mesh_name}")
                continue
            ...
            if not os.path.exists(data_path):
                print(f"[Warning] SDF data {data_path} not found, skipping {mesh_name}")
                continue
            ...
            # Flip anomalous large-magnitude negative values
            sdf_random_data[sdf_random_data < -1] = -sdf_random_data[sdf_random_data < -1]

            # Initialize Bernstein coefficients and RLS precision matrix
            ...
            print(f"[Info] mesh {mesh_name} trained, weight shape = {wb.shape}")
        ...
        torch.save(mesh_dict, os.path.join(self.model_path, f'BP_{self.n_func}.pt'))
        print(f"[Info] model saved at {self.model_path}/BP_{self.n_func}.pt")
