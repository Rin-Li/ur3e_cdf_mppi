...
class DataGenerator():
    def __init__(self,device):
        ...
        self.bp_sdf = BPSDF(8,-1.0,1.0,self.ur3e,device)
        self.model = torch.load(str(_ROOT / 'rdf' / 'models' / 'BP_8.pt'), weights_only=False)
        self.q_max = self.ur3e.theta_max
        self.q_min = self.ur3e.theta_min
        # device
        self.device = device

        # data generation
        self.workspace = [[-0.4,-0.4,0.0],[0.4,0.4,0.4]]
        self.n_disrete = 20         # total number of x: n_discrete**3
        self.batchsize = 50000       # batch size of q
    ...
    def given_x_find_q(self,x,q = None, batchsize = None,return_mask = False,epsilon = 1e-3):
        ...
        if q is None:
            q = torch.rand(batchsize,6).to(self.device)*(self.q_max-self.q_min)+self.q_min
        res = minimize(
            cost_function, 
            q, 
            method='l-bfgs', 
    ...
    def distance_q(self,x,q):
        ...
        if link_idx.min() == 0:
            ...
            d = torch.inf*torch.ones(Np,6).to(self.device)
    ...
    def generate_offline_data(self, save_path=None):
        if save_path is None:
            save_path = str(Path(__file__).resolve().parent / 'data')
